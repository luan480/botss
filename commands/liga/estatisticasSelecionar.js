...
async function mostrarSelecao(interaction) {
    const payload = {
        content: '',
        embeds: [new EmbedBuilder().setTitle('📊 ESTATÍSTICAS DA LIGA').setColor('#2ECC71').setDescription('Selecione **um jogador** abaixo para consultar as estatísticas da temporada atual.')],
...
async function mostrarJogador(interaction, userId) {
    ...
    const embed = new EmbedBuilder()
        ...
        .setColor('#2ECC71')
        .setDescription(
            `🏆 **${pontos} pts**  •  🥇 **${vitorias} vitórias**  •  🎮 **${partidas} partidas**\n` +
            `💀 **${kills} kills**  •  ☠️ **${mortes} mortes**  •  📈 **${winrate}% winrate**\n\n` +
            `🌍 **${continentes} continentes conquistados**`
        )
        .addFields({
            name: '🌍 Domínios',
            value: `🇪🇺 Europa: **${numero(c.europa)}** | 🌏 Ásia: **${numero(c.asia)}**\n🌍 África: **${numero(c.africa)}** | 🌎 Am. Norte: **${numero(c.amnorte)}**\n🌎 Am. Sul: **${numero(c.amsul)}** | 🌊 Oceania: **${numero(c.oceania)}**`,
            inline: false
        }, {
            name: '🏅 Extras',
            value: `🥉 3º lugares: **${numero(jogador.terceiroLugar)}** | ⚔️ Mais tropas: **${numero(jogador.maisTropas)}**`,
            inline: false
        })
