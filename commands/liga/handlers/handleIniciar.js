...
function contarPartidasRegistradasDoJogador(
    ...
    for (
        ...
        const jogadores =
            Array.isArray(partida.jogadoresBrutos)
...
function gerarPainelAoVivo(
    ...
    if (fase >= 2 && respostas.abates.length > 0) {
        ...
        respostas.abates.forEach(abate => {
            if (!abatesAgrupados[abate.matador]) {
                abatesAgrupados[abate.matador] = [];
            }

            abatesAgrupados[abate.matador].push(
                `<@${abate.vitima}>`
            );
        });

        const abatesStr =
            Object.entries(abatesAgrupados)
                .map(
                    ([matador, vitimas]) =>
    ...
    if (fase >= 3 && respostas.continentes.length > 0) {
        ...
        respostas.continentes.forEach(continente => {
            if (!contsAgrupados[continente.dono]) {
                contsAgrupados[continente.dono] = [];
            }
            ...
            contsAgrupados[continente.dono].push(
                `**${nomeContinente}**`
            );
        });

        const contStr =
            Object.entries(contsAgrupados)
                .map(
                    ([dono, conts]) =>
...
module.exports =
    async (
        ...
        try {
            ...
            const anexosUrls =
                Array.from(mensagemPrint.attachments.values())
            ...
            while (true) {
                ...
                // A partida aceita SOMENTE 6 jogadores.
                if (mentions.size === 6) {
            ...
            const continentesDispTotal =
                Object.entries(configPontos.continentes)
                    .map(([id, info]) => ({
                ...
                if (fase === 1) {
                    ...
                    if (int.customId === 'btn_confirmar_p1') {
                        if (
                            ...
                            await int.reply({
                                content: '❌ Preencha todas as 3 caixas antes de avançar!',
                                flags: MessageFlags.Ephemeral
                            });
                            continue;
                        }

                        if (respostas.vencedor === respostas.segundo) {
                            await int.reply({
                    ...
                    if (int.customId === 'add_abate_lote') {
                        ...
                        const intMatador = await painelMsg
                            .awaitMessageComponent({
                                filter: i => i.user.id === interaction.user.id,
                                time: 60000
                            })
                        ...
                        const intVitimas = await painelMsg
                            .awaitMessageComponent({
                                filter: i => i.user.id === interaction.user.id,
                                time: 60000
                            })
                    ...
                    if (int.customId === 'add_cont_lote') {
                        ...
                        const intConts = await painelMsg
                            .awaitMessageComponent({
                                filter: i => i.user.id === interaction.user.id,
                                time: 60000
                            })
                        ...
                        continentesDisp = continentesDisp.filter(
                            continente =>
                                !intConts.values.includes(continente.value)
                        );
            ...
            for (const [uid, d] of Object.entries(tabela)) {
                const partidasAntes =
                    Number(partidasAntesDaAtual[uid]) || 0;
                ...
                if (
                    ...
                    if (Date.now() < punicaoAtiva.expiraEm) {
                        ...
                        resumo +=
                            `🔹 <@${uid}>: **0 pts** *(Punição por Deserção)*\n`;
                    ...
                    if (
                        ...
                        if (
                            ...
                            try {
                                ...
                                if (memberDiscord) {
                                    const cargoAlvo =
                                        memberDiscord.guild.roles.cache.get(
                                            rankCorretoObj.id
                                        );
            ...
            if (respostas.abates.length > 0) {
                ...
                respostas.abates.forEach(abate => {
                    if (!abatesAgrupadosLog[abate.matador]) {
                        abatesAgrupadosLog[abate.matador] = [];
                    }

                    abatesAgrupadosLog[abate.matador].push(
                        `<@${abate.vitima}>`
                    );
                });

                logAbates =
                    Object.entries(abatesAgrupadosLog)
                        .map(
                            ([matador, vitimas]) =>
                                `⚔️ <@${matador}> eliminou: ${vitimas.join(', ')}`
                        )
            ...
            if (respostas.continentes.length > 0) {
                ...
                respostas.continentes.forEach(continente => {
                    if (!contsAgrupadosLog[continente.dono]) {
                        contsAgrupadosLog[continente.dono] = [];
                    }

                    const infoContinente =
                        configPontos.continentes[continente.cont];

                    contsAgrupadosLog[continente.dono].push(
                        `**${infoContinente?.nome || continente.cont}**`
                    );
                });

                logContinentes =
                    Object.entries(contsAgrupadosLog)
                        .map(
                            ([dono, conts]) =>
                                `🌍 <@${dono}> dominou: ${conts.join(', ')}`
                        )
