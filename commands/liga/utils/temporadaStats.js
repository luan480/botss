...
function timestampSnowflake(id) {
    ...
    try { return Number((BigInt(texto) >> 22n) + DISCORD_EPOCH); } catch { return null; }
}

function timestampDaPartida(partida, fallbackId) {
    for (const valor of [partida?.id, partida?.messageId, partida?.mensagemId, partida?.registroId, partida?.createdAt, partida?.criadoEm, partida?.data, fallbackId]) {
...
function criarPerfil(id) {
    return { id: String(id), partidas: 0, vitorias: 0, derrotas: 0, primeiroLugar: 0, segundoLugar: 0, kills: 0, mortes: 0, continentes: 0, europa: 0, asia: 0, africa: 0, amnorte: 0, amsul: 0, oceania: 0, pontos: 0, pontosGanhos: 0, pontosPerdidos: 0, warCoins: 0, winrate: 0 };
}
...
function jogadoresDaPartida(partida) {
    const ids = new Set();
    for (const jogador of partida?.jogadoresBrutos || []) { const id = idDe(jogador); if (id) ids.add(id); }
    for (const idOriginal of Object.keys(partida?.pontos || {})) { const id = idDe(idOriginal); if (id) ids.add(id); }
    for (const chave of ['vencedor', 'winner', 'ganhador', 'segundo', 'segundoLugar', 'runnerUp']) { const id = idDe(partida?.respostas?.[chave]); if (id) ids.add(id); }
    return [...ids];
}
...
function processar(porId, partida) {
    ...
    if (vencedor) { const j = garantir(porId, vencedor); j.vitorias++; j.primeiroLugar++; }
    ...
    for (const kill of (respostas.abates || respostas.kills || respostas.eliminacoes || [])) {
    ...
    for (const continente of (respostas.continentes || respostas.territorios || [])) {
        const id = idDe(continente?.dono || continente?.jogador || continente?.jogadorId || continente?.userId || continente?.conquistador);
        if (!id) continue;
        const j = garantir(porId, id); j.continentes++;
        const nome = String(continente?.cont || continente?.continente || continente?.territorio || '').toLowerCase().trim();
        if (nome === 'europa' || nome === 'europe') j.europa++;
        else if (nome === 'asia' || nome === 'ásia') j.asia++;
        else if (nome === 'africa' || nome === 'áfrica') j.africa++;
        else if (['amnorte','am_norte','america_do_norte','américa_do_norte','america-norte'].includes(nome)) j.amnorte++;
        else if (['amsul','am_sul','america_do_sul','américa_do_sul','america-sul'].includes(nome)) j.amsul++;
        else if (nome === 'oceania' || nome === 'oceânia') j.oceania++;
    }
    for (const [idOriginal, dados] of Object.entries(partida.pontos || {})) {
        const id = idDe(idOriginal); if (!id) continue;
        const j = garantir(porId, id);
        const pontos = dados && typeof dados === 'object' && !Array.isArray(dados) ? numero(dados.ptsLiga ?? dados.pontos ?? dados.pontuacao) : numero(dados);
        j.pontos += pontos; if (pontos >= 0) j.pontosGanhos += pontos; else j.pontosPerdidos += Math.abs(pontos);
        if (dados && typeof dados === 'object' && !Array.isArray(dados)) j.warCoins += numero(dados.wcRecebido ?? dados.warCoins ?? dados.wc);
    }
}
...
function calcular(inicioIso, pontuacaoAtual = null) {
    ...
    if (pontuacaoAtual && typeof pontuacaoAtual === 'object') for (const [idOriginal, dados] of Object.entries(pontuacaoAtual)) {
        const id = idDe(idOriginal); if (!id) continue;
        ...
        if (j.pontos === 0 && pontos !== 0) j.pontos = pontos;
    }
