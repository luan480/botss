...
// Estado temporário de cada contabilização. Evita colocar nomes de países
// grandes dentro dos customIds e permite vários usuários contabilizando.
...
function rankingPaises(dados) {
    ...
    for (const resultado of dados.resultados) {
        for (const [id, medalha] of [
            ...
            const chave = normalizar(dupla.pais);
            if (!ranking[chave]) {
                ranking[chave] = {
                    pais: dupla.pais,
                    ouro: 0,
