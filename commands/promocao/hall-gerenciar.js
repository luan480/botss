...
const CAMPOS_FIXOS = {
    liga: [
        ['nome', '🏆 Nome da liga', 'short'], ['ano', '📅 Ano', 'short'], ['descricao', '📝 Descrição', 'paragraph'],
        ['meses', '🗓️ Meses / vencedores', 'paragraph'], ['campeao', '🥇 Campeão', 'short'], ['vice', '🥈 Vice-campeão', 'short'],
        ['terceiro', '🥉 3º lugar', 'short'], ['premio', '🎁 Prêmio', 'paragraph'], ['participantes', '👥 Participantes', 'paragraph'],
        ['observacoes', '📌 Observações', 'paragraph'], ['imagem', '🖼️ Imagem / URL', 'short']
    ],
    eventos: [
        ['nome', '⚔️ Nome do evento', 'short'], ['vencedor', '🥇 Vencedor', 'short'], ['segundo', '🥈 2º lugar', 'short'],
        ['terceiro', '🥉 3º lugar', 'short'], ['participantes', '👥 Participantes', 'paragraph'], ['valor', '💰 Valor', 'short'],
        ['premio', '🎁 Prêmio', 'paragraph'], ['descricao', '📝 Descrição', 'paragraph'], ['data', '📅 Data', 'short'],
        ['horario', '🕐 Horário', 'short'], ['observacoes', '📌 Observações', 'paragraph'], ['imagem', '🖼️ Imagem / URL', 'short']
    ],
    records: [
        ['nome', '📊 Nome do record', 'short'], ['descricao', '📝 Descrição', 'paragraph'], ['linhas', '📈 Records', 'paragraph'],
        ['campeao', '🏆 Recordista', 'short'], ['valor', '📈 Valor', 'short'], ['premio', '🎁 Prêmio', 'paragraph'],
        ['observacoes', '📌 Observações', 'paragraph'], ['imagem', '🖼️ Imagem / URL', 'short']
    ],
    imperador: [
        ['nome', '👑 Nome do Imperador', 'short'], ['ano', '📅 Ano', 'short'], ['descricao', '📝 Descrição', 'paragraph'],
        ['meses', '🗓️ Meses / vencedores', 'paragraph'], ['campeao', '👑 Campeão', 'short'], ['vice', '🥈 Vice-campeão', 'short'],
        ['terceiro', '🥉 3º lugar', 'short'], ['premio', '🎁 Prêmio', 'paragraph'], ['participantes', '👥 Participantes', 'paragraph'],
        ['observacoes', '📌 Observações', 'paragraph'], ['imagem', '🖼️ Imagem / URL', 'short']
    ]
...
function criarModal(categoria, id, registro, pagina) {
    ...
    const modal = new ModalBuilder()
        .setCustomId(`hallmgr_edit:${categoria}:${id}:${pagina}:${lista.length}`)
...
function aplicar(registro, key, value) {
    const v = texto(value);
    if (!v) return;
    if (v === '-') { delete registro[key]; return; }
...
async function atualizarMural(guild) {
    try {
        ...
        if (typeof painel.atualizarMural === 'function') return await painel.atualizarMural(guild);
    } catch (e) { console.error('[HALL] mural:', e); }
    return false;
}
function criarEmbed(categoria, r) {
    const e = new EmbedBuilder().setColor('#C9A227').setTitle('🏛️ HALL DA FAMA — GERENCIAMENTO')
        .setDescription(`**${limitar(nomeRegistro(r), 256)}**\nCategoria: **${categoria.toUpperCase()}**`);
    for (const [key, label] of obterCampos(categoria, r).slice(0, 25)) {
...
async function processarModal(interaction) {
    ...
    const campos = lista[pagina - 1] || [];
    for (const [key] of campos) aplicar(found.registro, key, interaction.fields.getTextInputValue(key.slice(0, 100)));
    try {
    ...
    return interaction.editReply(`✅ **${nomeRegistro(found.registro)}** foi atualizado completamente.`);
}
async function confirmarRemocao(interaction, categoria, id) {
    ...
    await atualizarMural(interaction.guild);
    return interaction.editReply({ content: `🗑️ **${nome}** foi removido do Hall da Fama.`, embeds: [], components: [] });
}
async function remover(interaction, categoria, id) {
    ...
    const row = new ActionRowBuilder().addComponents(
        new ButtonBuilder().setCustomId(`hallmgr_confirm_delete_${categoria}_${id}`).setLabel('Confirmar remoção').setEmoji('🗑️').setStyle(ButtonStyle.Danger),
        new ButtonBuilder().setCustomId('hallmgr_cancel_delete').setLabel('Cancelar').setEmoji('❌').setStyle(ButtonStyle.Secondary)
    );
    return interaction.reply({ content: `⚠️ **Confirma a remoção de ${nomeRegistro(found.registro)}?**`, embeds: [criarEmbed(categoria, found.registro)], components: [row], flags: MessageFlags.Ephemeral });
}
async function processarInteracao(interaction) {
    ...
    if (interaction.isModalSubmit()) {
        if (interaction.customId.startsWith('hallmgr_edit:')) return processarModal(interaction);
        return;
    }
    if (!interaction.isButton()) return;
    if (interaction.customId === 'hallmgr_cancel_delete') return interaction.update({ content: '↩️ Remoção cancelada.', embeds: [], components: [] });
    if (interaction.customId.startsWith('hallmgr_confirm_delete_')) {
        const p = interaction.customId.split('_');
        return confirmarRemocao(interaction, p[3], p.slice(4).join('_'));
    }
    if (interaction.customId.startsWith('hallmgr_edit_')) {
        const p = interaction.customId.split('_');
...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addStringOption(o => o.setName('acao').setDescription('Ação').setRequired(true).addChoices({ name: '✏️ Editar', value: 'editar' }, { name: '🗑️ Remover', value: 'remover' }))
