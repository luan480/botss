...
function limitar(v, n) { const s = texto(v); return s.length <= n ? s : s.slice(0, n - 3) + '...'; }
function permitido(interaction) { return interaction.memberPermissions?.has(PermissionFlagsBits.Administrator); }
...
async function atualizarMural(guild) {
    ...
    } catch (e) { console.error('[HALL] Falha ao atualizar mural:', e); }
}
function valorCampo(r, chave) { const v = r?.[chave]; return v === null || v === undefined ? '' : String(v); }
function modalEditar(categoria, id, r) {
    const modal = new ModalBuilder().setCustomId(`hall_edit_submit_${categoria}_${id}`).setTitle('✏️ Editar registro do Hall');
    const campos = [
    ...
    for (const [idCampo, label, estilo, max, obrigatorio] of campos) {
        modal.addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId(idCampo).setLabel(label).setStyle(estilo).setRequired(obrigatorio).setMaxLength(max).setValue(limitar(valorCampo(r, idCampo), max))));
    }
    return modal;
}
function embedResumo(categoria, r) {
    return new EmbedBuilder().setColor('#C9A227').setTitle('🏛️ Gerenciamento do Hall da Fama')
...
async function handler(interaction) {
    ...
    if (interaction.isButton()) {
        if (interaction.customId === 'hall_manage_cancel') return interaction.update({ content: '↩️ Remoção cancelada.', embeds: [], components: [] });
        const parts = texto(interaction.customId).split('_');
        if (parts[0] !== 'hall' || parts[1] !== 'manage') return;
    ...
    if (interaction.isModalSubmit() && interaction.customId.startsWith('hall_edit_submit_')) {
        ...
        const categoria = parts[3], id = parts.slice(4).join('_');
        const d = carregar();
        const achado = encontrar(d, categoria, id);
        if (!achado.registro) return interaction.reply({ content: '❌ Esse registro não existe mais.', flags: MessageFlags.Ephemeral });
...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addStringOption(o => o.setName('categoria').setDescription('Categoria do registro.').setRequired(true).addChoices(
            { name: '🏆 Liga', value: 'liga' }, { name: '⚔️ Eventos', value: 'eventos' },
            { name: '📊 Records', value: 'records' }, { name: '👑 Imperador', value: 'imperador' }
        ))
        .addStringOption(o => o.setName('id').setDescription('ID do registro exibido no Hall.').setRequired(true)),
    async execute(interaction) {
        if (!permitido(interaction)) return interaction.reply({ content: '❌ Apenas administradores podem gerenciar o Hall.', flags: MessageFlags.Ephemeral });
        const categoria = interaction.options.getString('categoria'), id = interaction.options.getString('id');
        const d = carregar(), achado = encontrar(d, categoria, id);
        if (!achado.registro) return interaction.reply({ content: `❌ Não encontrei o registro \`${id}\` na categoria **${categoria}**.`, flags: MessageFlags.Ephemeral });
        ...
        if (!interaction.client.__hallManagerAttached) {
            interaction.client.__hallManagerAttached = true;
            interaction.client.on('interactionCreate', i => {
                if ((i.isButton() && (i.customId.startsWith('hall_manage_'))) || (i.isModalSubmit() && i.customId.startsWith('hall_edit_submit_'))) handler(i).catch(e => console.error('[HALL] Erro no gerenciamento:', e));
            });
        }
