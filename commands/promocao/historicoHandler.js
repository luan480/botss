...
async function nomeDoJogador(interaction, jogador) {
    const nomeSalvo = texto(jogador?.nome || jogador?.displayName || jogador?.username);
    if (nomeSalvo && !idValido(nomeSalvo)) return nomeSalvo;
    const id = jogador?.id || jogador?.userId || jogador?.jogadorId || (idValido(nomeSalvo) ? nomeSalvo : null);
    if (!id) return nomeSalvo || 'Jogador';
    try {
        const membro = await interaction.guild?.members?.fetch(String(id));
        if (membro) return membro.displayName || membro.user.globalName || membro.user.username;
    } catch {}
    try {
        const usuario = await interaction.client?.users?.fetch(String(id));
        if (usuario) return usuario.globalName || usuario.username || `Jogador ${id}`;
    } catch {}
    return `Jogador ${id}`;
}
...
function ranking(r) {
    return estatisticasDeRegistro(r).filter(x => x && (x.id || x.userId || x.jogadorId || x.nome)).map(x => ({ ...x, id: x.id || x.userId || x.jogadorId })).sort((a, b) => (Number(b.pontos) || 0) - (Number(a.pontos) || 0));
}
function linhaTop10(lista) {
    if (!lista.length) return '*Nenhuma classificação arquivada.*';
    return lista.slice(0, 10).map((j, i) => `${['🥇','🥈','🥉'][i] || `**${i + 1}º**`} ${j.nome && !idValido(j.nome) ? j.nome : mencao(j.id) || 'Jogador'} — **${Number(j.pontos) || 0} pts**`).join('\n');
}
function formatar(categoria, r) {
    if (r?.__legacy) return { titulo: `${categoria === 'liga' ? '🏆 Liga' : '👑 Imperador'} — ${r.ano}`, descricao: '📜 **Registro histórico consolidado**', campos: [campo('🗓️ CAMPEÕES / REGISTROS', r.linhas.join('\n'))], stats: [], liga: false };
    const liga = categoria === 'liga' || r?.categoria === 'liga';
    const stats = estatisticasDeRegistro(r), top = ranking(r), campos = [];
...
function embedStats(r, p, t, nomes = {}) {
    ...
    slice.forEach((j, i) => {
        ...
        const id = j.id || j.userId || j.jogadorId;
