...
// Configuração local: NÃO precisa estar no GitHub.
// Na StabilityHost, deixe o config.json ao lado deste index.js.
let config = {};
try {
    ...
    for (const file of fs.readdirSync(dir)) {
        ...
        try {
            ...
            if (!nome || client.commands.has(nome)) {
                if (nome) console.warn(`[CMD] Ignorado comando duplicado: ${nome}`);
                continue;
            }
...
client.once(Events.ClientReady, async c => {
    ...
    try {
        c.user.setPresence({
            activities: [{ name: config.presence || 'WAR', type: ActivityType.Playing }],
            status: 'online'
        });
    } catch {}
    ...
    try {
        ...
        if (guild) {
            ...
            console.error(`❌ Servidor autorizado ${ID_SERVIDOR_AUTORIZADO} não encontrado.`);
        }
    } catch (erro) {
    ...
    try {
        const promotion = require('./commands/promocao/promotionHandler.js');
        if (typeof promotion === 'function') promotion(c);
    } catch (erro) {
        console.error('❌ Falha no PromotionHandler:', erro);
    }

    try {
        const syncEngine = require('./commands/promocao/syncEngine.js');
        if (typeof syncEngine.executarVarreduraCanal === 'function') {
            await syncEngine.executarVarreduraCanal(c);
            console.log('✅ Varredura e recuperação automática de prints concluída.');
        }
    } catch (erro) {
        console.error('❌ Falha na varredura automática:', erro);
    }

    const handlers = [
        ['./commands/promocao/reactionAddHandler.js', 'Reaction Handler'],
        ['./commands/adm/adminLogHandler.js', 'Admin Log'],
        ['./commands/voz/voiceControlHandler.js', 'Voice Control'],
        ['./commands/adm/temporaryVoiceHandler.js', 'Temporary Voice'],
        ['./commands/adm/temporaryMuteHandler.js', 'Temporary Mute'],
        ['./commands/adm/weeklyReportHandler.js', 'Relatórios'],
        ['./commands/economy/economyTextHandler.js', 'Economy'],
        ['./commands/adm/autoResponseHandler.js', 'Auto Response'],
        ['./commands/adm/antiNukeHandler.js', 'Anti-Nuke'],
        ['./commands/adm/onboardingSyncHandler.js', 'Onboarding']
    ];
    ...
    for (const [arquivo, nome] of handlers) {
        try {
            ...
            console.error(`❌ Falha no ${nome}:`, erro);
        }
    }
});
...
client.on(Events.InteractionCreate, async interaction => {
    ...
    try {
        ...
        if (customId.startsWith('hist_') || customId.startsWith('hall_')) {
            return await require('./commands/promocao/historicoHandler.js')(interaction, client);
        }

        if (customId.startsWith('edit_match_')) {
            return await handleReverter(client, interaction, pontuacaoPath, partidasPath);
        }

        if (customId.startsWith('ticket_')) {
            return await require('./commands/ticket/buttonRouter.js')(interaction, client);
        }

        if (customId.startsWith('stt_')) {
            return await require('./commands/promocao/statusHandler.js')(interaction, client);
        }

        if (customId.startsWith('rank_')) {
            return await require('./commands/promocao/rankingHandler.js')(interaction, client);
        }
        ...
        if (customId.startsWith('tvoice_')) {
            return await require('./commands/adm/tempVoiceButtonHandler.js')(interaction, client);
        }

        if (customId.startsWith('vcall_select_')) {
            if (!interaction.values?.length) {
                return interaction.reply({ content: '❌ Nenhum usuário selecionado.', flags: MessageFlags.Ephemeral });
            }
        ...
        if (
            ...
            ['iniciar_contabilizacao', 'ver_ranking', 'ver_todos_competidores', 'registrar', 'add_abate', 'fim_abates', 'add_cont', 'fim_cont'].includes(customId) ||
            customId.startsWith('sel_') ||
            customId.startsWith('reset_')
        ) {
            return await require('./commands/liga/buttons.js')(client, interaction);
        }
    } catch (erro) {
...
if (!TOKEN) {
    console.error('❌ TOKEN NÃO ENCONTRADO. Configure "token" no config.json ou DISCORD_TOKEN/BOT_TOKEN/TOKEN no ambiente.');
    process.exit(1);
}
