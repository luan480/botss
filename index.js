    ...
    for (const file of fs.readdirSync(dir)) {
        ...
        try {
            ...
            if (!nome || client.commands.has(nome)) {
                if (nome) {
                    console.warn('[CMD] Ignorado comando duplicado: ' + nome);
                }
...
client.once(Events.ClientReady, async c => {
    ...
    try {
        ...
        for (const [, guildData] of guilds) {
            ...
            if (
                ...
                const guild =
                    c.guilds.cache.get(guildData.id);
    ...
    for (
        const [arquivo, nome] of handlers
    ) {
...
// O config.json da StabilityHost continua sendo a fonte principal.
// As variáveis de ambiente acima servem apenas como fallback.
